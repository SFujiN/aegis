...
class Candidate {
 public:
  /**
   * @brief Candidate constructor that takes in the party they represent and the candidate's name.
   *
  ...
  void negateStatus();
    /**
   * @brief Overload operator for comparisons between Candidates' number of ballots.
   * 
   * @param r, represents the Candidate to compare "this" to.
   * 
   * @return Returns true or false to represent which candidate has a larger number of ballots.
   */
  bool operator<(Candidate& r);

 private:
  char party; ///< A char representing the name of the party the Candidate belongs to.
  std::string name; ///< A string that holds the name of the Candidate.
  bool status = true; ///< A boolean that represents whether the Candidate is still in the running. Defaults to true.
  std::vector<Ballot> ballots; ///< A vector of Ballots that holds all ballots currently voting for this Candidate.
};
